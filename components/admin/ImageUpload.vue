<template>
  <div class="space-y-4">
    ...
    <div v-if="modelValue" class="relative inline-block">
      <img :src="modelValue" :alt="label" class="w-full max-w-xs h-32 object-cover rounded-xl border border-border-primary">
      <button
        type="button"
    ...
    <div class="space-y-3">
      ...
      <div v-if="showExistingImages && existingImages.length > 0">
        ...
        >
          ...
          <span>{{ showGallery ? 'Masquer' : 'Choisir parmi les images existantes' }}</span>
        </button>
        
        <div v-if="showGallery" class="mt-3 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 max-h-40 overflow-y-auto">
          <div
